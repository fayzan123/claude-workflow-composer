...
`completionCriteria` is always present in the node — an empty string signals the user hasn't filled it in yet. The canvas always renders the field and templates pre-populate it.
...
- `completionCriteria` — required plain-English description of what "done" looks like for this agent. The canvas always shows this field labeled "How will you know this agent succeeded?" and templates pre-populate it with sensible defaults. When non-empty, the exporter injects a `## Completion Criteria` block into the agent's system prompt (see Agent Files section). When empty string, no `## Completion Criteria` section is emitted. Directly addresses the reliability gap identified by the CCW creator: agents need explicit acceptance criteria before they start.
