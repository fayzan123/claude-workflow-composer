...
describe('validatePlan', () => {
  ...
  it('drops a malformed reuse but keeps the phase', () => {
    const p = validatePlan({ ...valid, phases: [{ id: 'p1', intent: 'x', stepIndexes: [0], reuse: { kind: 'skill' } }] }, 3)
    expect(p).not.toBeNull()
    expect(p!.phases[0].reuse).toBeUndefined()
  })

  it('keeps a well-formed reuse', () => {
    const p = validatePlan({ ...valid, phases: [{ id: 'p1', intent: 'x', stepIndexes: [0], reuse: { kind: 'skill', slug: 'tdd', coversStepIndexes: [0], why: 'fits' } }] }, 3)
    expect(p!.phases[0].reuse?.slug).toBe('tdd')
  })
})
